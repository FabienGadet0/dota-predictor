...
func getPredictions(w http.ResponseWriter, r *http.Request) {
	...
	var data []Data
	result := config.DB.Raw(`select p.*, g.radiant_team , g.dire_team, g.radiant_score, g.dire_score, g.start_date from prediction as p left join games as g on g.match_id = p.match_id order by inserted_date desc limit 100 offset ?`, offset).Scan(&data)
	if result.Error != nil {
		w.WriteHeader(http.StatusNotFound)
